...
export default function Footer() {
  ...
  const socialLinks = [
    { icon: FaWhatsapp,   href: settings.whatsappNumber  ? `https://wa.me/${settings.whatsappNumber}` : null, label: "WhatsApp" },
    { icon: FaFacebook,   href: settings.facebookPageUrl || null, label: "Facebook" },
    { icon: FaInstagram,  href: settings.instagramUrl    || null, label: "Instagram" },
    { icon: FaYoutube,    href: settings.youtubeChannelUrl || null, label: "YouTube" },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div className="flex flex-col gap-4">
            ...
            <p className="text-choco-300 text-sm leading-relaxed font-hind">
              {settings.tagline}
            </p>
          ...
          <div className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-2">
              {QUICK_LINKS.map((link) => (
                <li key={link.to}>
                  <Link to={link.to}
                    className="text-choco-300 hover:text-caramel-400 transition-colors duration-200 text-sm font-hind">
                    {link.label}
                  </Link>
                </li>
              ))}
              <li>
                <button onClick={handleOrder}
                  className="text-choco-300 hover:text-caramel-400 transition-colors duration-200 text-sm text-left font-hind">
                  {t.footer.orderNow}
                </button>
              </li>
            </ul>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2 text-sm text-choco-300">
              {settings.whatsappNumber && (
                <a href={`https://wa.me/${settings.whatsappNumber}`} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-2 hover:text-caramel-400 transition-colors font-hind">
                  <FaWhatsapp size={16} />
                  +{settings.whatsappNumber}
                </a>
              )}
            ...
            <button onClick={handleOrder} className="btn-primary text-sm w-fit mt-2 font-hind">
              {t.footer.orderNow}
            </button>
          </div>
        </div>
