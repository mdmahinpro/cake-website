...
export default function GalleryCard({ item, index, onClick }: GalleryCardProps) {
  return (
    <motion.div
      ...
      whileHover={{ y: -4, boxShadow: "0 12px 32px rgba(0,190,255,0.18)" }}
      onClick={onClick}
    >
      {/* Square image fill */}
        ...
        >
          {item.imageUrl ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-choco-700 to-choco-900 flex items-center justify-center">
              <span className="font-playfair text-3xl font-bold text-caramel-400/25 select-none">
                SD
              </span>
            </div>
          )}
