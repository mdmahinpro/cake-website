  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.20) 50%, transparent 100%)",
        }}
      />

      {/* Top badges */}
      <div className="absolute top-3 left-3 right-3 flex justify-between items-start pointer-events-none">
        <span className="px-2 py-0.5 rounded-full text-xs font-medium text-white bg-caramel-400/90 capitalize">
          {item.category}
        </span>
        {(item as CakeItem & { youtubeUrl?: string }).youtubeUrl && (
          <span className="px-2 py-0.5 rounded-full text-xs font-medium text-white bg-red-600/90">
            Video
          </span>
        )}
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 translate-y-1 group-hover:translate-y-0 transition-transform duration-300">
        <p className="text-xs md:text-sm text-white/90 line-clamp-2 font-poppins pr-12">
          {item.caption}
        </p>
      </div>

      {/* Order button */}
      <div className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 scale-75 group-hover:scale-100 transition-all duration-200">
