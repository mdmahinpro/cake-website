...
export default function DeliveredSection() {
  ...
  const delivered = gallery.filter((item) => item.type === "delivered" && item.imageUrl).slice(0, 6);
    ...
    <section className="py-20 bg-gradient-to-b from-choco-800 to-choco-900 transition-colors duration-700">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {delivered.map((item, i) => (
            <motion.div key={item.id}
              ...
              <div className="relative overflow-hidden" style={{ aspectRatio: "4/3" }}>
                <motion.img src={item.imageUrl} alt={item.caption}
                  className="w-full h-full object-cover"
                  whileHover={{ scale: 1.05 }} transition={{ duration: 0.5 }} loading="lazy" />
              ...
              <div className="p-4 flex flex-col gap-3 flex-1">
                <p className="text-sm text-white/90 font-hind leading-snug line-clamp-2">{item.caption}</p>
                {item.review && (
                  <div className="flex gap-2 rounded-2xl p-3"
                    ...
                    <p className="text-xs text-caramel-200 italic leading-relaxed font-hind">{item.review}</p>
                  </div>
                )}
              </div>
            </motion.div>
        ...
        <div className="mt-10 flex justify-center">
          <button onClick={() => navigate("/gallery")} className="btn-outline font-hind">
            {t.delivered.seeAll}
          </button>
        </div>
      </div>
