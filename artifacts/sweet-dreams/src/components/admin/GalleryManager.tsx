...
export default function GalleryManager({ filterDelivered = false }: GalleryManagerProps) {
  ...
  function handleFile(file: File) {
    if (file.size > 1.5 * 1024 * 1024) {
      alert("Image is larger than 1.5MB. It will still work but may slow the page.");
    }
  ...
  return (
    <div className="max-w-5xl mx-auto flex flex-col gap-6">
            ...
            <div className="flex-1 flex flex-col gap-4">
              ...
              <div>
                ...
                {form.imageSource === "upload" ? (
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => setUrlTestImg(form.imageUrl)}
              ...
              <div>
                ...
                <div className="relative">
                  <textarea
                    value={form.caption}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, caption: e.target.value.slice(0, 200) }))
                    }
                    rows={3}
                    className="input-dark resize-none"
                    placeholder="Describe this cake..."
                  />
                  <span className="absolute bottom-2 right-3 text-xs text-choco-300">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`relative w-12 h-6 rounded-full transition-colors duration-300 flex-shrink-0 ${
                    form.featured ? "bg-caramel-400" : "bg-choco-700"
                  }`}
                >
                  <span
                    className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform duration-300 ${
                      form.featured ? "translate-x-6" : "translate-x-0.5"
                    }`}
                  />
                </button>
                <span className="text-sm text-caramel-300">
              ...
              <div>
                ...
                <input
                  ...
                  className="input-dark"
                />
                <p className="text-xs text-choco-300 mt-1">
                  Adds a "Watch Video" button in gallery lightbox
                </p>
              </div>
              ...
              <div className="rounded-2xl overflow-hidden bg-choco-800 border border-caramel-800/30">
                <div className="aspect-square bg-choco-700 relative">
                  {(finalImage || urlTestImg) ? (
                    <img
                      src={finalImage || urlTestImg || ""}
                      className="w-full h-full object-cover"
                      alt="preview"
                    />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-4xl text-choco-500">
                      🎂
                    </div>
                  )}
                  {form.category && (
      ...
      <div className="card-dark p-6 rounded-3xl">
        ...
        <p className="text-xs text-choco-300 mb-4">
              ...
              <div key={item.id} className="rounded-2xl overflow-hidden relative bg-choco-700">
                <div className="aspect-square relative">
                  ...
                  {(item as CakeItem & { youtubeUrl?: string }).youtubeUrl && (
                    <span className="absolute bottom-1.5 left-1.5 px-1.5 py-0.5 rounded-full text-[10px] bg-red-600/90 text-white leading-none">
                      Video
                    </span>
                  )}
                </div>
                <div className="bg-choco-800 p-2 flex items-center justify-between gap-1">
                  ...
                  <div className="flex gap-1 flex-shrink-0">
                    <button
                      ...
                      className="text-caramel-400 hover:text-caramel-300 p-1 transition-colors"
                    >
                      <MdEdit size={16} />
                    </button>
                    <button
      ...
      <AnimatePresence>
        {deleteTarget && (
          ...
          >
            <motion.div
              className="card-dark p-6 rounded-3xl max-w-sm w-full"
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <button
                  ...
                  className="text-choco-300 hover:text-white"
                >
                  <MdClose size={20} />
                </button>
              </div>
              <p className="text-sm text-choco-300 mb-6">
